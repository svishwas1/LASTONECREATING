...
void pre_auton(void)
{
  ...
  {                             // Changing the names below will only change their names on the
    ...
    { // Tap the brain screen to cycle through autons.
      case 0:
        Brain.Screen.printAt(50, 50, "Testing Auton");
        break;
      case 1:
...
void lbcontrol(){
  ...
  isPressed = true;
  if (lbstate == 0){
    wallstake.spinTo(-195, deg, 200, rpm, false);
    lbstate = 1;
  }
  else if  (lbstate == 1){
    wallstake.spinTo(-700, deg, 200, rpm, false);
    lbstate = 2;
  }
  else {
    wallstake.spinTo(-100, deg, 200, rpm, false);
    lbstate = 0;
  } 
}
...
void usercontrol(void)
{
  ...
  while (1)
  {
    ...
    //both spin
    Controller1.ButtonY.pressed(lbcontrol);
    ...
    //} else if (Controller1.ButtonL1.pressing()){
      //wallstake.spin(fwd, 100, percent);
    //} else {
      //wallstake.stop();
    //}
    //
